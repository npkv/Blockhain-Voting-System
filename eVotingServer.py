...
def authenticateLogin():
    ...
    conn1 = pyodbc.connect(Constants.connString)
    cur1 = conn1.cursor()
...
def processChangePassword():
    ...
    conn1 = pyodbc.connect(Constants.connString, autocommit=True)
    cur1 = conn1.cursor()
...
def RegisterAsVoter():
    ...
    cur3.commit()
    age_min = datetime.date.today() - relativedelta(years=120)
    age_max = datetime.date.today() - relativedelta(years=18)
    return render_template('RegisterAsVoter.html', constituencies=constituencies, age_min=age_min, age_max=age_max)
...
def RegisterAsContestant():
    ...
    age_min = datetime.date.today() - relativedelta(years=120)
    age_max = datetime.date.today() - relativedelta(years=18)
    return render_template('RegisterAsContestant.html', constituencies=constituencies, age_min=age_min, age_max=age_max)
...
def ApproveVoterListing():
    ...
    if searchData == None:
        searchData = "";
    conn2 = pyodbc.connect(Constants.connString, autocommit=True)
    cursor = conn2.cursor()
...
def CastMyVoteListing():
    ...
    sqlcmd3 = """SELECT DISTINCT electionID, constituencyID FROM ContestantElectionDetails WHERE EXISTS 
                    ...
                    VoterMaster.constituencyID = ContestantElectionDetails.constituencyID AND emailID = '"""+globalUserObject.emailID+"')"
    print(sqlcmd3)
    cursor3.execute(sqlcmd3)
    cerecords = []
...
def ProcessCastMyVoteOperation():
    ...
    compiled_contract_path = '../../../{2}/build/contracts/{1}Contract.json'
    deployed_contract_address = Constants.contract_address
    ...
    tx_hash = contract.functions.perform_transactions({0}).transact({{'from': accounts[0]}})
...
def VoteOperation():
    ...
    conn2 = pyodbc.connect('Driver={SQL Server};Server=COMPUTER-PC\WISEN123;Integrated_Security=true;Database=E-Voting', autocommit=True)
    cursor = conn2.cursor()
    ...
    while True:
        ...
        if not dbrow:
            break
        row = PoliticalDataModel(dbrow[0], dbrow[1])
...
def ProcessVoteOperation():
    ...
    conn3 = pyodbc.connect('Driver={SQL Server};Server=COMPUTER-PC\WISEN123;Integrated_Security=true;Database=E-Voting', autocommit=True)
    cur3 = conn3.cursor()
...
def ProcessRoleOperation():
    ...
    if operation == "Delete" :
        conn4 = pyodbc.connect('Driver={SQL Server};Server=COMPUTER-PC\WISEN123;Integrated_Security=true;Database=E-Voting', autocommit=True)
        cur4 = conn4.cursor()
...
def UserListing():
    ...
    if searchData == None:
        searchData = "";
    conn = pyodbc.connect(Constants.connString, autocommit=True)
    cursor = conn.cursor()
...
def UserOperation():  
    ...
    conn4 = pyodbc.connect(Constants.connString, autocommit=True)
...
def VoteDataListing():
    ...
    if searchData == None:
        searchData = "";
    conn = pyodbc.connect('Driver={SQL Server};Server=COMPUTER-PC\WISEN123;Integrated_Security=true;Database=E-Voting', autocommit=True)
    cursor = conn.cursor()
    sqlcmd1 = "SELECT voteID, author, keywords, cloudProviderID, isDownloadedToServer, videoFileName FROM VoteData WHERE author LIKE '"+searchData+"%'"
    cursor.execute(sqlcmd1)
    records = []
    ...
    while True:
        ...
        temp = str(dbrow[3])
        sqlcmd3 = "SELECT * FROM PoliticalPartyName WHERE cloudProviderID = '"+temp+"'"
        print(sqlcmd3)
        cursor3.execute(sqlcmd3)
        ...
        if cloudProviderRow:
           cloudProviderObj = PoliticalDataModel(cloudProviderRow[0], cloudProviderRow[1])
        else:
           print("Cloud Provider Row is Not Available")
        
        row = VoteDataModel(dbrow[0],dbrow[1],dbrow[2],cloudProviderModel=cloudProviderObj, isDownloadedToServer=dbrow[4], videoFileName=dbrow[5])
        
        records.append(row)
    cursor.close()
    conn.close()
    return render_template('VoteDataListing.html', records=records, searchData=searchData)

@app.route("/VoteDataOperation")
def VoteDataOperation():
    ...
    conn4 = pyodbc.connect('Driver={SQL Server};Server=COMPUTER-PC\WISEN123;Integrated_Security=true;Database=E-Voting', autocommit=True)
    cursor4 = conn4.cursor()
    sqlcmd4 = "SELECT * FROM PoliticalPartyName"
    cursor4.execute(sqlcmd4)
    while True:
        ...
        cloudProviderDDObj = PoliticalDataModel(cloudProviderDDrow[0], cloudProviderDDrow[1])
    ...
    if operation != "Create" :
        ...
        conn2 = pyodbc.connect('Driver={SQL Server};Server=COMPUTER-PC\WISEN123;Integrated_Security=true;Database=E-Voting', autocommit=True)
        cursor = conn2.cursor()
        sqlcmd1 = "SELECT * FROM VoteData WHERE voteID = '"+unqid+"'"
        ...
        while True:
            ...
            if cloudproviderrow:
                cloudproviderObj = PoliticalDataModel(cloudproviderrow[0], cloudproviderrow[1])
            row = VoteDataModel(dbrow[0], dbrow[1], dbrow[2], dbrow[3], dbrow[4], dbrow[5],dbrow[6], dbrow[7],cloudProviderModel = cloudproviderObj)
            
    print("cloudProviderDDObj", len(cloudProviderDDList))    
    return render_template('VoteDataOperation.html', row = row, operation=operation, cloudProviderDDList=cloudProviderDDList )


@app.route("/ProcessVoteDataOperation", methods=['POST'])
def ProcessVoteDataOperation():
    ...
    initialize()
    print("ProcessVoteDataOperation")
    operation = request.form['operation']
    if operation != "Delete" :
    ...
    sqlcmd = ""
    if operation == "Create" :
        sqlcmd = "INSERT INTO VoteData (author, keywords, mediaFormat, closedCaption, contentData, cloudProviderID, fileSize, videoFileName) VALUES ('"+author+"', '"+keywords+"', '"+mediaFormat+"', '"+closedCaption+"', '"+contentData+"', '"+cloudProviderID+"', '"+str(fileSize)+"', '"+fileName+"')"
        
    if operation == "Edit" :
        print("edit inside")
        sqlcmd = "UPDATE VoteData SET author = '"+author+"', keywords = '"+keywords+"', mediaFormat = '"+mediaFormat+"', closedCaption = '"+closedCaption+"', contentData = '"+contentData+"', cloudProviderID = '"+cloudProviderID+"', fileSize= '"+str(fileSize)+"', videoFileName = '"+fileName+"' WHERE voteID = '"+unqid+"'" 
    if operation == "Delete" :
        sqlcmd = "DELETE FROM VoteData WHERE voteID = '"+unqid+"'" 
    print(operation, sqlcmd)
    if sqlcmd == "" :
        return redirect(url_for('Error')) 
    cur3.execute(sqlcmd)
    ...
    return redirect(url_for('VoteDataListing')) 
    #return render_template('UserListing.html',  msg="Cloud Provider Information Successfully Created")
...
def FTPDownload():
    ...
    conn2 = pyodbc.connect('Driver={SQL Server};Server=COMPUTER-PC\WISEN123;Integrated_Security=true;Database=E-Voting', autocommit=True)
    cursor = conn2.cursor()
    sqlcmd1 = "SELECT videoFileName FROM VoteData WHERE voteID = '"+unqid+"'"
    cursor.execute(sqlcmd1)
    dbrow = cursor.fetchone()
    if dbrow:
        fileName = dbrow[0]
        ...
        cur3 = conn3.cursor()
        sqlcmd = ""
        sqlcmd = "UPDATE VoteData SET isDownloadedToServer = 1 WHERE voteID = '"+unqid+"'" 
        cur3.execute(sqlcmd)
        cur3.commit()
        ftp = ftplib.FTP('101.99.74.37', 'Student1','3Aj73#lc')
        print("fileName", fileName)
...
def ProcessBlockchainGeneration():
    ...
    if blocksCreated != 0 :
        connx = pyodbc.connect(Constants.connString, autocommit=True)
        ...
        if dbrowx:
            voteID = dbrowx[0]
            conny = pyodbc.connect(Constants.connString, autocommit=True)
...
def AnalysisVoterGenderWiseReport():
    ...
    conn2 = pyodbc.connect(Constants.connString, autocommit=True)
    cursor = conn2.cursor()
    sqlcmd1 = "SELECT gender, Count(*) FROm VoterMaster WHERE isApproved = 1 GROUP BY gender ORDER BY gender"
    print(sqlcmd1)
    cursor.execute(sqlcmd1)
    records = []
